...
class USER {
    ...
    public function loan($tc, $sender_name, $reqtype, $lamount, $income, $occup, $employer, $reason) {
        try {

            $stmt = $this->conn->prepare("INSERT INTO loan(tc,sender_name,reqtype,lamount,income,occup,employer,reason) 
			                                             VALUES(:tc, :sender_name, :reqtype, :lamount, :income, :occup, :employer, :reason)");
            $stmt->bindparam(":tc", $tc);
            $stmt->bindparam(":sender_name", $sender_name);
    ...
    public function creditcards($tc, $sender_name, $cardtype, $card_no, $expdate, $cardpin, $cvv, $cardname) {
        try {

            $stmt = $this->conn->prepare("INSERT INTO creditcards(tc,sender_name,cardtype,card_no,expdate,cvv,cardpin,cardname) 
			                                             VALUES(:tc, :sender_name, :cardtype, :card_no, :expdate, :cvv, :cardpin, :cardname)");
            $stmt->bindparam(":tc", $tc);
            $stmt->bindparam(":sender_name", $sender_name);
    ...
    public function logout() {
     ...
     function send_mail($email, $messag, $subject) {
        ...
        $mail->SMTPAuth = true;
        $mail->SMTPSecure = "";
        $mail->Host = "mail.danamontrust.com";
        $mail->Port = 26;
        $mail->AddAddress($email);
        $mail->Username = "noreply@danamontrust.com";
        $mail->Password = "SendCoDe@0iN2";
        $mail->SetFrom('noreply@danamontrust.com', 'Danamon Trust');
        $mail->AddReplyTo("noreply@danamontrust.com", "Danamon Trust");
